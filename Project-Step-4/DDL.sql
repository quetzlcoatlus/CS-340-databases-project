...
-- Date: 02/18/2026
-- Adapted from Prof. Michael Curry's CS 340 course for PL/SQL wrapper
-- sp_moviedb.sql file which facilitates resetting the database
-- Source URL: https://canvas.oregonstate.edu/courses/2031764/assignments/10323339?module_item_id=26243440

DROP PROCEDURE IF EXISTS sp_load_usv_fleet_management_system_db;
DELIMITER //
CREATE PROCEDURE sp_load_usv_fleet_management_system_db()
BEGIN
    -- Create database and use it
    -- Uncomment if NOT using CS340 database
    -- CREATE DATABASE IF NOT EXISTS `name`;
    -- USE `name`;

    -- Enable dropping of the tables
    SET FOREIGN_KEY_CHECKS = 0;

    -- Essentially copy paste from the DDL file
    ...
    */
    ...
    -- Reset Foreign Key Checks now that we're done
    SET FOREIGN_KEY_CHECKS = 1;
    COMMIT;
END //

DELIMITER ;
-- Resets the schema back to the original state
-- CALL sp_load_usv_fleet_management_system_db()